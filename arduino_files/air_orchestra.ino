/*
 * Air Orchestra — Ring Firmware v1.1 (Access Point Mode)
 * ESP32 + MPU6050 → WiFi (SoftAP) → WebSocket → Browser
 *
...
// ─────────────────────────────────────────────────────────────────
// CONFIG — Access Point Settings
// ─────────────────────────────────────────────────────────────────
#define RING_ID       "ring1"
#define AP_SSID       "AirOrchestra_Ring" // The Wi-Fi name your Mac will connect to
#define AP_PASS       "12345678"          // Password (must be at least 8 characters)
#define WS_PORT       81
...
void setup() {
  ...
  esp_bt_controller_disable(); //Bluetooth disable kr diya
  setCpuFrequencyMhz(80); //CPU freq kum kr di
  ...
  // ── WiFi Access Point (SoftAP) Setup ─────────────────────────
  Serial.printf("Starting Wi-Fi Hotspot: \"%s\"...\n", AP_SSID);
  ...
  // The default IP address for an ESP32 SoftAP is usually 192.168.4.1
